...
class DoctorReportWizard(models.TransientModel):
    ...
    def action_print_report(self):
        ...
        return (self.env.ref('hr_hospital.action_report_doctor_card').
                report_action(self.doctor_ids))
    ...
    def action_preview_report(self):
        ...
        action = self.env.ref(
            'hr_hospital.action_report_doctor_card'
        ).read()[0]
        action['report_type'] = 'qweb-html'
        # Pass the IDs to the action, similar to report_action()
        action['context'] = dict(
            self.env.context, active_ids=self.doctor_ids.ids
            )
        return action
